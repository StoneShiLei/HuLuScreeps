...
export default class BodyAutoConfigUtil {
    static bodyAutoConfigs = {
        harvester: BodyAutoConfigUtil.getBodyConfig(
         ...
         */
        worker: BodyAutoConfigUtil.getBodyConfig(
         ...
         * 负责转移基地资源的 creep
         */
        transporter: BodyAutoConfigUtil.getBodyConfig(
            { [CARRY]: 2, [MOVE]: 1 },
            { [CARRY]: 3, [MOVE]: 2 },
            { [CARRY]: 4, [MOVE]: 2 },
            { [CARRY]: 5, [MOVE]: 3 },
    ...
    static getBodyConfig(...bodySets: [ BodySet, BodySet, BodySet, BodySet, BodySet, BodySet, BodySet, BodySet]): BodyConfig {
        ...
        Object.keys(config).map((level, index) => {
            config[level as keyof BodyConfig] = BodyAutoConfigUtil.calcBodyPart(bodySets[index])
        })
        return config
    }
