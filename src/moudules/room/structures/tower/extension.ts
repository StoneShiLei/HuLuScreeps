...
import FillTowerTask from "moudules/room/taskController/task/transporterTask/fillTowerTask"
...
export default class TowerExtension extends StructureTower {
    ...
    private dailyWork(): void {
        ...
        // else {
        //     if (this.room.controller && this.room.controller.level >= TOWER_FILL_WALL_LEVEL) this.commandFillWall()
        // }
    ...
    private commandRepair(): boolean {
        ...
        if (!this.room._damagedStructure) {
            ...
            if (damagedStructures.length > 0) {
                const ds = this.pos.findClosestByRange(damagedStructures)
                this.room._damagedStructure = ds ?? 1
            }
            else {
    ...
    private commandFillWall(): boolean {
        ...
        if (!focusWall || (focusWall && Game.time >= focusWall.endTime)) {
            ...
            const walls = this.room.find<StructureRampart | StructureWall>(FIND_STRUCTURES,{filter:s => (s.structureType == STRUCTURE_WALL || s.structureType == STRUCTURE_RAMPART) && s.hits < s.hitsMax} )
        ...
        // 获取墙壁
        if (focusWall && !targetWall) targetWall = Game.getObjectById(focusWall.id)
    ...
    private findEnemy(searchInterval: number = 1): (Creep|PowerCreep)[] {
        ...
        this.room._enemys = this.room.find(FIND_HOSTILE_CREEPS, {
            filter: WhiteList.whiteListFilter
        })
    ...
    private fire(enemys: (Creep|PowerCreep)[]): ScreepsReturnCode {
        if (!enemys || enemys.length <= 0) return ERR_NOT_FOUND
        const pos = this.pos.findClosestByRange(enemys)
        if(!pos) return ERR_NOT_FOUND
    ...
    private requireEnergy(lowerLimit: number = 900): void {
        if (this.store[RESOURCE_ENERGY] <= lowerLimit && !this.room.transportController.hasTask('fillTower')) {
            this.room.transportController.addTask(new FillTowerTask(this.id))
        }
    }
}
