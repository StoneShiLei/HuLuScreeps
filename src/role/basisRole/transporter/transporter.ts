...
export default class TransporterConfig implements RoleConfig{
    ...
    getResource?(creep:Creep):boolean{
        if(!creep.memory.data.transporterData) return false
        const { sourceID,workRoom } = creep.memory.data.transporterData
    ...
    workWithTarget(creep: Creep): boolean {
        if(!creep.memory.data.transporterData) return true
        const { workRoom } = creep.memory.data.transporterData
